...
class CommandAutoSuggest(AutoSuggest):
    """Inline auto-suggest for slash commands: shows the first argument name as a ghost text."""
    ...
    def get_suggestion(self, buffer: Buffer, document: Document) -> Suggestion | None:
        """Return a ghost-text suggestion for the first argument of a known slash command."""
        text = document.text
...
class UnifiedCompleter(Completer):
    """Tab-completion provider for both slash commands (/) and document references (@)."""
    ...
    def update_prompts(self, prompts: list):
        """Replace the current list of available slash-command prompts."""
        self.prompts = prompts
        self.prompt_dict = {prompt.name: prompt for prompt in prompts}

    def update_resources(self, resources: list):
        """Replace the current list of available document resource IDs."""
        self.resources = resources

    def get_completions(self, document, complete_event):
        """Yield completions for @ references and / slash commands based on cursor context."""
        text = document.text
...
class CliApp:
    """Interactive CLI application with prompt-toolkit: handles input, completion, and the main loop."""
    ...
    async def initialize(self):
        """Fetch resources and prompts from the MCP server to populate completions."""
        await self.refresh_resources()
        await self.refresh_prompts()

    async def refresh_resources(self):
        """Reload available document IDs from the MCP server into the completer."""
        try:
    ...
    async def refresh_prompts(self):
        """Reload available slash-command prompts from the MCP server into the completer."""
        try:
    ...
    async def run(self):
        """Start the interactive prompt loop; exits on Ctrl+C."""
        while True:
            try:
