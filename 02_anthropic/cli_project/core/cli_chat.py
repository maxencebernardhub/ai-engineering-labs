...
class CliChat(Chat):
    """Extends Chat with document retrieval and slash-command handling via an MCP document server."""
    ...
    async def list_prompts(self) -> list[Prompt]:
        """Return all slash-command prompts registered on the document MCP server."""
        return await self.doc_client.list_prompts()

    async def list_docs_ids(self) -> list[str]:
        """Return the list of available document IDs from the MCP server."""
    ...
    async def get_prompt(self, command: str, doc_id: str) -> list[PromptMessage]:
        """Resolve a slash command into a list of prompt messages for the given document."""
        return await self.doc_client.get_prompt(command, {"doc_id": doc_id})

    async def _extract_resources(self, query: str) -> str:
        """Parse @mentions in the query and return the referenced document contents as XML."""
    ...
    async def _process_command(self, query: str) -> bool:
        """If the query is a slash command, resolve it into messages and return True; else False."""
    ...
    async def _process_query(self, query: str):
        """Build and append the user message, injecting @-referenced document content as context."""
        ...
        Don't refer to or mention the provided context in any way - just use it to
        inform your answer.
        """
...
def convert_prompt_message_to_message_param(
    ...
    """Convert a single MCP PromptMessage into an Anthropic MessageParam dict."""
...
def convert_prompt_messages_to_message_params(
    ...
    """Convert a list of MCP PromptMessages into Anthropic MessageParam dicts."""
